...
export default function SessionPage() {
  ...
  async function createSession() {
    ...
    const res = await fetch("/api/admin/session", {
      ...
      body: JSON.stringify({
        ...
        rootTitel: sessionRootTitel,
        rootBeschreibung: sessionRootBeschreibung,
        rootContext: sessionRootContext,
    ...
    if (res.ok) {
      ...
      setSessionTitle("");
      setSessionRootTitel("");
      setSessionRootBeschreibung("");
      setSessionRootContext("");
  ...
  async function setAsDefault(id: string) {
    if (
      !confirm(
        "Diese Session als aktive Standard-Session für Benutzer setzen? Die aktuell aktive Session wird archiviert.",
      )
    )
      return;
    setSaving(true);
  ...
  async function removeMedia(url: string) {
    ...
    await fetch("/api/admin/session/media", {
      method: "DELETE",
      headers: headers(),
      body: JSON.stringify({ sessionId: currentSession.id, url }),
    });
    await reloadSessions();
    setUploadingMedia(false);
  }
  ...
  async function resetTree() {
    ...
    if (
      !confirm(
        `Wirklich den kompletten Baum "${currentSession.title || currentSession.treeId}" zurücksetzen? Alle Knoten werden gelöscht!`,
      )
    )
      return;
    setSaving(true);
      ...
      {showCreateSession && !currentSession && (
        <div style={{ display: "grid", gap: 10 }}>
          <div style={s.row}>
            <input
              value={sessionTreeId}
              onChange={(e) => setSessionTreeId(e.target.value)}
              placeholder="Tree ID (z.B. dream-v2)"
          ...
          <input
            value={sessionRootTitel}
            onChange={(e) => setSessionRootTitel(e.target.value)}
            placeholder="Titel (max 2 Wörter)"
            style={s.input}
          />
          <input
            value={sessionRootBeschreibung}
            onChange={(e) => setSessionRootBeschreibung(e.target.value)}
            placeholder="Beschreibung (Stichworte)"
            style={s.input}
          />
          <textarea
            value={sessionRootContext}
            onChange={(e) => setSessionRootContext(e.target.value)}
            placeholder="Kontext (Szenenbeschreibung)"
            rows={3}
          ...
          <textarea
            ...
            placeholder="System-Prompt für AI-Generierung..."
            rows={5}
            style={{ ...s.input, resize: "vertical" as const, fontFamily: "monospace", fontSize: 12 }}
          />
          <div style={s.row}>
            <span style={{ fontSize: 12, opacity: 0.7 }}>Modell:</span>
            <select
              value={sessionModelName}
              onChange={(e) => setSessionModelName(e.target.value)}
          ...
          <div style={s.row}>
            <button
              ...
              disabled={loading || !sessionTreeId || !sessionRootTitel || !sessionRootBeschreibung}
            >
              Erstellen
            </button>
            <button style={s.btnSmall} onClick={() => setShowCreateSession(false)}>
              Abbrechen
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {currentSession.status === "active" && currentSession.startedAt && (
            <div style={s.countdown}>
              ...
              <span style={s.deadlineText}>
                Deadline:{" "}
                {sessionDeadline(currentSession)?.toLocaleString("de-CH", {
                  day: "2-digit",
                  month: "2-digit",
                  year: "numeric",
                  hour: "2-digit",
          ...
          <div style={{ ...s.row, marginTop: 0 }}>
            {currentSession.status === "draft" && (
              <>
                <button
                  style={{ ...s.btnSmall, background: "rgba(96,165,250,0.2)" }}
                  onClick={() => sessionAction(currentSession.id, "start")}
                >
                  Starten
                </button>
                <button
                  style={{ ...s.btnSmall, background: "rgba(255,59,92,0.2)" }}
            ...
            {currentSession.status === "active" && (
              <button
                style={{ ...s.btnSmall, background: "rgba(74,222,128,0.2)" }}
                onClick={() => sessionAction(currentSession.id, "archive")}
              >
                Archivieren
              </button>
            )}
            {currentSession.status === "archived" && (
              <button
                style={{ ...s.btnSmall, background: "rgba(96,165,250,0.2)" }}
                onClick={() => setAsDefault(currentSession.id)}
                disabled={saving}
              >
                Als Standard setzen (Aktivieren)
              </button>
            )}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                position: "relative",
                background: currentSession.discoveryEnabled ? "rgba(52,199,89,0.8)" : "rgba(255,255,255,0.15)",
            ...
            >
              <div
                style={{
                  ...
                  position: "absolute",
                  top: 3,
                  left: currentSession.discoveryEnabled ? 23 : 3,
                  transition: "left 0.2s",
                }}
              />
            </button>
            <span style={{ fontSize: 13, fontWeight: 800 }}>
              Entdeckung {currentSession.discoveryEnabled ? "aktiv" : "pausiert"}
            </span>
          </div>
          ...
          <div style={s.row}>
            ...
            <button style={s.btn} onClick={saveSessionConfig} disabled={saving}>
              Speichern
            </button>
          </div>
          ...
          {showResetTree && (
            ...
            >
              ...
              <div style={{ fontSize: 12, opacity: 0.7 }}>
                Dies löscht alle existierenden Knoten und erstellt einen neuen Root-Knoten.
              </div>
              <div style={{ fontSize: 12, opacity: 0.6, fontWeight: 800 }}>Neuer Root-Knoten</div>
              <input
                value={resetRootTitel}
                onChange={(e) => setResetRootTitel(e.target.value)}
                placeholder="Titel (max 2 Wörter)"
                style={s.input}
              />
              <input
                value={resetRootBeschreibung}
                onChange={(e) => setResetRootBeschreibung(e.target.value)}
                placeholder="Beschreibung (Stichworte)"
                style={s.input}
              />
              <textarea
                value={resetRootContext}
                onChange={(e) => setResetRootContext(e.target.value)}
                placeholder="Kontext (Szenenbeschreibung)"
                rows={3}
                style={{ ...s.input, resize: "vertical" as const }}
              />
              <div style={s.row}>
                <button
                  style={{ ...s.btn, background: "rgba(255,59,92,0.8)", color: "white" }}
                  onClick={resetTree}
                  disabled={saving || !resetRootTitel || !resetRootBeschreibung}
                >
          ...
          {otherSessions.map((sess) => (
            ...
            >
              ...
              <span style={{ opacity: 0.5, fontSize: 11 }}>
                {sess._count.nodes} Knoten · {sess._count.votes} Stimmen
              </span>
              <span style={{ opacity: 0.5, fontSize: 11 }}>
                {sess.endedAt
                  ? new Date(sess.endedAt).toLocaleDateString("de-CH", {
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
                    })
