...
export default function ResultsPage() {
  ...
  // resolve option details for leaderboard
  useEffect(() => {
    if (!data) return;

    (async () => {
      const ids = data.leaderboard.map((x) => x.optionId).filter(Boolean);
      ...
      <div style={s.container}>
          ...
          {!voterId || !treeId ? (
            ...
            <div style={s.muted}>
              ...
              <div style={{ marginTop: 10 }}>
                <button style={s.btn} onClick={() => router.push("/start")}>
                  Start
                </button>
              </div>
            </div>
            ...
            <div style={s.myPick}>
              ...
              <div style={s.myText}>
                ...
                <div style={s.rowBtns}>
                  <button style={s.btn} onClick={() => router.push(`/o/${encodeURIComponent(myOption.id)}`)}>
                    Öffnen
                  </button>
                  <button style={s.btnGhost} onClick={() => router.push("/start")}>
                    Ändern
                  </button>
                </div>
              </div>
        ...
        <section style={s.card}>
          ...
          {loading ? (
            ...
            <div style={s.muted}>Keine Daten verfügbar.</div>
          ) : rows.length === 0 ? (
            ...
            <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
              {rows.map((r, idx) => (
                <div key={r.optionId} style={s.row}>
                  ...
                  <button
                    style={s.rowBtn}
                    onClick={() => router.push(`/o/${encodeURIComponent(r.optionId)}`)}
