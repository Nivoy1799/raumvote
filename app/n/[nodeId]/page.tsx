  ...
  useEffect(() => {
    ...
    fetchNodePage(nodeId).then((data) => {
      ...
      if (!data.node.generated && voterId) {
        setGenerating(true);
        generateChildren(nodeId, voterId)
          .then((gen) => {
            setNode(gen.node);
            setLeft(gen.left);
            setRight(gen.right);
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `/api/tree/node/images?leftId=${encodeURIComponent(left.id)}&rightId=${encodeURIComponent(right.id)}`,
        );
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      const [l1, l2, v, c1, c2, cc1, cc2] = await Promise.all([
        fetch(
          `/api/like/status?sessionId=${encodeURIComponent(sessionId)}&optionId=${encodeURIComponent(left.id)}&voterId=${encodeURIComponent(voterId)}`,
        ).then((r) => r.json()),
        fetch(
          `/api/like/status?sessionId=${encodeURIComponent(sessionId)}&optionId=${encodeURIComponent(right.id)}&voterId=${encodeURIComponent(voterId)}`,
        ).then((r) => r.json()),
        fetch(
          `/api/vote/status?sessionId=${encodeURIComponent(sessionId)}&voterId=${encodeURIComponent(voterId)}`,
        ).then((r) => r.json()),
        fetch(
          `/api/like/count?sessionId=${encodeURIComponent(sessionId)}&optionId=${encodeURIComponent(left.id)}`,
        ).then((r) => r.json()),
        fetch(
          `/api/like/count?sessionId=${encodeURIComponent(sessionId)}&optionId=${encodeURIComponent(right.id)}`,
        ).then((r) => r.json()),
        fetch(
          `/api/comment/count?sessionId=${encodeURIComponent(sessionId)}&optionId=${encodeURIComponent(left.id)}`,
        ).then((r) => r.json()),
        fetch(
          `/api/comment/count?sessionId=${encodeURIComponent(sessionId)}&optionId=${encodeURIComponent(right.id)}`,
        ).then((r) => r.json()),
      ]);
      ...
      <main style={styles.shell}>
        ...
        <div style={{ ...styles.frame, touchAction: "none" }} {...carouselBind}>
          ...
          {arrivalInfo && (
            ...
            >
              <div style={{ fontSize: r.fontSize.body, fontWeight: 800 }}>Richtung: {arrivalInfo}</div>
          ...
          {!isFocused && (
            ...
            >
              ...
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                style={{ opacity: 0.5, animation: "rv-nudge-left 2s ease-in-out infinite" }}
              >
                <path
                  d="M15 4l-8 8 8 8"
                  stroke="white"
              ...
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                style={{ opacity: 0.5, animation: "rv-nudge-right 2s ease-in-out infinite" }}
              >
            ...
            {options.map((opt, i) => (
              <div
                ...
                style={{
                  ...
                  overflow: "hidden",
                  animation:
                    !isFocused && !isDragging ? `rv-center-wobble 3s ease-in-out ${i * 0.4}s infinite` : undefined,
                }}
              >
                <Image
                  src={opt.mediaUrl || placeholderUrl}
                  alt={opt.titel}
                  fill
                  priority
                ...
                >
                  <div style={{ display: "flex", alignItems: "flex-end", gap: r.spacing.small }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: r.fontSize.title + 2, fontWeight: 900, letterSpacing: -0.3 }}>
                        {opt.titel}
                      </div>
                    ...
                    <ActionRail
                      ...
                      items={[
                        ...
                        {
                          icon: faComment,
                          count: commentCounts[i],
                          ariaLabel: "Kommentare",
                          onClick: () => openComments(opt.id),
                        },
      ...
      <div style={styles.frame} {...swipe.bind()}>
        ...
        <section style={styles.split}>
          ...
          <div role="button" tabIndex={0} style={styles.half} onClick={() => navigate(left)}>
            <Image
              src={left.mediaUrl || placeholderUrl}
              alt={left.titel}
              fill
              priority
              style={{ objectFit: "cover" }}
            ...
            <div style={styles.overlay}>
              <div style={{ display: "flex", alignItems: "flex-end", gap: r.spacing.small }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: r.fontSize.body - 1, opacity: 0.8, marginTop: isMed ? 2 : 6 }}>
                    {left.beschreibung}
                  </div>
                </div>
                <ActionRail
                  ...
                  items={[
                    {
                      ...
                      count: likeCountLeft,
                      ariaLabel: "Gefällt mir",
                      onClick: () => toggleLike(left.id),
                    },
                    {
                      icon: faCheckToSlot,
                      active: votedOptionId === left.id,
                      activeColor: "#60a5fa",
                      ariaLabel: "Abstimmen",
                      onClick: () => vote(left.id),
                    },
                    {
                      icon: faComment,
                      count: commentCountLeft,
                      ariaLabel: "Kommentare",
                      onClick: () => openComments(left.id),
                    },
          ...
          <div role="button" tabIndex={0} style={styles.half} onClick={() => navigate(right)}>
            <Image
              src={right.mediaUrl || placeholderUrl}
              alt={right.titel}
              fill
              priority
              style={{ objectFit: "cover" }}
                  ...
                  items={[
                    {
                      icon: faHeart,
                      active: likedRight,
                      count: likeCountRight,
                      ariaLabel: "Gefällt mir",
                      onClick: () => toggleLike(right.id),
                    },
                    {
                      icon: faCheckToSlot,
                      active: votedOptionId === right.id,
                      activeColor: "#60a5fa",
                      ariaLabel: "Abstimmen",
                      onClick: () => vote(right.id),
                    },
                    {
                      icon: faComment,
                      count: commentCountRight,
                      ariaLabel: "Kommentare",
                      onClick: () => openComments(right.id),
                    },
